...
describe('getSelectorFromElement', () => {
  ...
  describe('should target closest atomic element when targetAtomicElement: true', () => {
    beforeEach(() => {
      if (isIE()) {
    ...
    it('based on their tags', () => {
      expect(getSelector('<button><span target><span></button>', { targetAtomicElement: true })).toBe('BODY>BUTTON')
      expect(getSelector('<a><span target><span></a>', { targetAtomicElement: true })).toBe('BODY>A')
      expect(getSelector('<select><option target><option></select>', { targetAtomicElement: true })).toBe('BODY>SELECT')
    })

    it('based on their role', () => {
      expect(getSelector('<div role="link"><span target><span></a>', { targetAtomicElement: true })).toBe('BODY>DIV')
    })

    it('based on [aria-label]', () => {
      expect(getSelector('<div aria-label="foo"><span target><span></a>', { targetAtomicElement: true })).toBe(
        'BODY>DIV'
      )
    })

    it('based on [alt]', () => {
      expect(getSelector('<div  alt="foo"><span target><span></a>', { targetAtomicElement: true })).toBe('BODY>DIV')
    })

    it('based on [name]', () => {
      expect(getSelector('<div name="foo"><span target><span></a>', { targetAtomicElement: true })).toBe('BODY>DIV')
    })

    it('based on [title]', () => {
      expect(getSelector('<div title="foo"><span target><span></a>', { targetAtomicElement: true })).toBe('BODY>DIV')
    })
  })
  ...
  function getSelector(
    ...
    options?: { stopRecurringWhenUnique?: boolean; targetAtomicElement?: boolean; onlyPrefixWithSemanticTag?: boolean },
