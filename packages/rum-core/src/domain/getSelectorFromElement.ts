...
const ATOMIC_ELEMENTS_SELECTORS = [
  [
    'button',
    'a',
    '[role=button]',
...
export function getSelectorFromElement(
  ...
  actionNameAttribute: string | undefined,
  { stopRecurringWhenUnique = false, targetAtomicElement = false, onlyPrefixWithSemanticTag = false } = {}
) {
  let targetElementSelector = ''
  let element: Element | null = targetElement
  const prefix = onlyPrefixWithSemanticTag ? getSemanticTagName : getTagNameSelector

  if (targetAtomicElement && targetElement.closest) {
    const closestClickableElement = targetElement.closest(ATOMIC_ELEMENTS_SELECTORS.join(','))
